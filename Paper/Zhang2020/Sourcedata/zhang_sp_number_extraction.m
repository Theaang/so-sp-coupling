...
first_dimension = squeeze(all_dc.sp_numbers_dc(1, :, :, :, :));
...
for participant = 1:size(all_dc.sp_numbers_dc, 5)
    for stage = 1:2 % nREM2 and SWS
        % Sum the third dimension for each 1:4 range
        third_dimension_sum = sum(all_dc.sp_numbers_dc(1, :, 1:4, :, participant), 3);
...
odd_columns_to_keep = [1, 2:3:73]; % For odd rows
even_columns_to_keep = [1, 3:3:73]; % For even rows
...
writetable(T_filtered, 'spindle_number.csv');