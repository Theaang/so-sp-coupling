...
knitr::kable(data.frame("Phase" = sch_phase, "Amplitude" = cor1, "Strength" = cor2, "SPcSO" = cor3, "SOcSP" = cor4), format = "markdown", caption = "Schreiner Coupling and Memory Pearson's r Correlation Table", row.names = FALSE)
```
...
knitr::kable(effect_size, format = "markdown", caption = "Denis 2021a CP Phase and Memory Pearson's r Correlation Table")
```
...
cortable("Denis 2021a", 2, flip = FALSE, "nREM2" = cor_n2, "nREM3" = cor_n3)
```
...
cortable("Denis 2021a", 3, flip = FALSE, "nREM2" = cor_n2, "nREM3" = cor_n3)
```
```{r}
#| warning: false
#| message: false
...
cortable("Denis 2021a", 4, flip = FALSE, "nREM2" = cor_n2, "nREM3" = cor_n3)
```
...
cor_slow <- c(
  ...
  "Parietal" = cor(behaviour_slow$change_pres ~ slow_percent_avg$POavg, use = "complete"))
...
#| message: false
## Import source data from Kurz 2023
Denis2021 <- read_csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Kurz2023/Sourcedata/dataDRMdevelopment.csv", show_col_types = FALSE)
#> view(Kurz2023_raw)
```
...
## knitr::kable(sch_phase,format ="markdown",caption ="Mylonas 2020 CP Phase and Memory Pearson's r Correlation Table",row.names = FALSE)
```
...
knitr::kable(data.frame("Phase" = mylonas_phase, "Amplitude" = cor1, "Strength" = cor2, "SPcSO" = cor3, "SOcSP" = cor4), format = "markdown", caption = "Mylonas 2020 Coupling and Memory Pearson's r Correlation Table", row.names = FALSE)
```
