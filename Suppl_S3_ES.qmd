...
eeg_download <- "https://github.com/Theaang/so-sp-coupling/raw/main/Paper/Donnelly2022/Sourcedata/sleep_study_eeg_summary_data.rds"
...
Donnelly_beh <- Donnelly_behrow |>
  filter(group == "Sib") |>
  mutate(retention = (accC_morning-accC_evening)/accC_evening) |>
...
Donnellyphase_avg <- Donnellyphase |>
  rowwise() |>
  mutate(
    Favg = (mean(c(F3, F4, Fz)) * pi) / 180 + 0.5*pi,
    Cavg = (mean(c(C3, C4, Cz)) * pi) / 180 + 0.5*pi,
    POavg = (mean(c(P3, P4, Pz, O1, O2)) * pi) / 180 + 0.5*pi)
...
Hahn_ampad <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Hahn2022/Sourcedata/amplitude_adolescent.csv")
Hahn_ampya <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Hahn2022/Sourcedata/amplitude_adult.csv")
Hahn_strad <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Hahn2022/Sourcedata/strength_adolescent.csv")
Hahn_strya <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Hahn2022/Sourcedata/strength_adult.csv")
#> view(Hahn_phasead)
#> view(Hahn_phaseya)
```
...
#| echo: false
cor_ad <- c(
  Frontal = mean(Hahn_ampad$rho_baseskillchange[Hahn_ampad$Electrode %in% c("F3", "F4", "Fz")]),
  Central = mean(Hahn_ampad$rho_baseskillchange[Hahn_ampad$Electrode %in% c("C3", "C4", "Cz")]),
  "Parietal and Occipital" = mean(Hahn_ampad$rho_baseskillchange[Hahn_ampad$Electrode %in% c("P3", "P4", "Pz", "O1", "O2")])
)

cor_ya <- c(
  Frontal = mean(Hahn_ampya$rho_baseskillchange[Hahn_ampya$Electrode %in% c("F3", "F4", "Fz")]),
  Central = mean(Hahn_ampya$rho_baseskillchange[Hahn_ampya$Electrode %in% c("C3", "C4", "Cz")]),
  "Parietal and Occipital" = mean(Hahn_ampya$rho_baseskillchange[Hahn_ampya$Electrode %in% c("P3", "P4", "Pz", "O1", "O2")])
)
effect_size <- data.frame("Adolescent" = cor_ad, "Young Adult" = cor_ya)
knitr::kable(effect_size, format = "markdown", caption = "Hahn 2022 SP Amplitude and Memory Circular Linear Correlation Table")
```
...
#| message: false
#| include: false
Hahn_strad_avg_raw <- Hahn_strad |>
rowwise() |>
mutate(
 Favg = (mean(c(F3, F4, Fz))),
 Cavg = (mean(c(C3, C4, Cz))),
 POavg = (mean(c(P3, P4, Pz, O1, O2))))
Hahn_strad_avg <- remove_outliers(Hahn_strad_avg_raw, scale_columns =  c("Favg","Cavg", "POavg"))
## view(Hahn_strad_avg)
Hahn_strya_avg_raw <- Hahn_strya |>
rowwise() |>
mutate(
 Favg = (mean(c(F3, F4, Fz))),
 Cavg = (mean(c(C3, C4, Cz))),
 POavg = (mean(c(P3, P4, Pz, O1, O2))))
Hahn_strya_avg <- remove_outliers(Hahn_strya_avg_raw, scale_columns =  c("Favg","Cavg", "POavg"))
## view(Hahn_strya_avg)
```
...
#| message: false
#| echo: false
# Calculate correlation coefficients between coupling strength and memory in each channel location for the adolescent group
cor_ad <- c(
  "Frontal" = cor(Hahn_strad_avg$retention ~ Hahn_strad_avg$Favg, use = "complete"),
  "Central" = cor(Hahn_strad_avg$retention ~ Hahn_strad_avg$Cavg, use = "complete"),
  "Parietal and Occipital" = cor(Hahn_strad_avg$retention ~ Hahn_strad_avg$POavg, use = "complete"))

# Calculate correlation coefficients between coupling strength and memory in each channel location for the young adult group
cor_ya <- c(
  "Frontal" = cor(Hahn_strya_avg$retention ~ Hahn_strya_avg$Favg, use = "complete"),
  "Central" = cor(Hahn_strya_avg$retention ~ Hahn_strya_avg$Cavg, use = "complete"),
  "Parietal and Occipital" = cor(Hahn_strya_avg$retention ~ Hahn_strya_avg$POavg, use = "complete"))

# Create the table
cortable("Hahn 2022", 3, flip = FALSE, "Adolescent" = cor_ad, "Young Adult" = cor_ya)
```
...
Solano_amplitude <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Solano2022/Sourcedata/solano_amplitude.csv")
...
#| message: false
#| include: false
n2_amp_avg_raw <- Solano_amplitude |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz_n2))),
 Cavg = (mean(c(Cz_n2, C3_n2, C4_n2))),
 Pavg = (mean(c(Pz_n2, P3_n2, P4_n2))))
n3_amp_avg_raw <- Solano_amplitude |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz_n3))),
 Cavg = (mean(c(Cz_n3, C3_n3, C4_n3))),
 Pavg = (mean(c(Pz_n3, P3_n3, P4_n3))))
```
...
#| message: false
#| echo: false
n2_amp_avg <- remove_outliers(n2_amp_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))
n3_amp_avg <- remove_outliers(n3_amp_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))

# Calculate correlation coefficients between coupling strength and memory in each channel location
cor_n2 <- c(
  "Frontal" = cor(retention ~ Favg, use = "complete", data = n2_amp_avg),
  "Central" = cor(retention ~ Cavg, use = "complete", data = n2_amp_avg),
  "Parietal" = cor(retention ~ Pavg, use = "complete", data = n2_amp_avg))

cor_n3 <- c(
  "Frontal" = cor(retention ~ Favg, use = "complete", data = n3_amp_avg),
  "Central" = cor(retention ~ Cavg, use = "complete", data = n3_amp_avg),
  "Parietal" = cor(retention ~ Pavg, use = "complete", data = n3_amp_avg))

# Create the table
cortable("Solano 2022", 2, flip = FALSE, "N2" = cor_n2, "N3" = cor_n3)

favstats(n2_amp_avg$Favg)
favstats(n2_amp_avg$Cavg)
favstats(n2_amp_avg$Pavg)
favstats(n3_amp_avg$Favg)
favstats(n3_amp_avg$Cavg)
favstats(n3_amp_avg$Pavg)
```
...
#| message: false
#| include: false
fast2_str_avg_raw <- Solano_strength |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz2fast))),
 Cavg = (mean(c(C32fast, Cz2fast, C42fast))),
 Pavg = (mean(c(P32fast, Pz2fast, P42fast))))
fast3_str_avg_raw <- Solano_strength |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz3fast))),
 Cavg = (mean(c(C33fast, Cz3fast, C43fast))),
 Pavg = (mean(c(P33fast, Pz3fast, P43fast))))
slow2_str_avg_raw <- Solano_strength |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz2slow))),
 Cavg = (mean(c(C32slow, Cz2slow, C42slow))),
 Pavg = (mean(c(P32slow, Pz2slow, P42slow))))
slow3_str_avg_raw <- Solano_strength |>
rowwise() |>
mutate(
 Favg = (mean(c(Fz3slow))),
 Cavg = (mean(c(C33slow, Cz3slow, C43slow))),
...
#| message: false
#| echo: false
fast2_str_avg <- remove_outliers(fast2_str_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))
fast3_str_avg <- remove_outliers(fast3_str_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))
slow2_str_avg <- remove_outliers(slow2_str_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))
slow3_str_avg <- remove_outliers(slow3_str_avg_raw, scale_columns =  c("Favg","Cavg","Pavg"))
...
#| message: false
#| include: false
## Import source data from Zhang 2020
Zhang2020 <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Zhang2020/Sourcedata/Zhang2020.csv")
Zhangpct <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Zhang2020/Sourcedata/zhang_percentage.csv")
```
...
## Coupling Phase Preprocessing
## Calculate the mean coupling phase for each electrode location cluster
Zhang2020_avg <- Zhang2020 |>
rowwise() |>
mutate(
 Favg_amp = (mean(c(F3, F4, Fz))),
 Cavg_amp = (mean(c(C3, C4, Cz))),
 POavg_amp = (mean(c(P3, P4, Pz, O1, O2))),
 Favg_phase = (mean(c(F3_phase, F4_phase)) + 90),
 Cavg_phase = (mean(c(C3_phase, C4_phase)) + 90),
 POavg_phase = (mean(c(P3_phase, P4_phase, O1_phase, O2_phase)) + 90),
 Favg_mi = (mean(c(F3_MI, F4_MI))),
 Cavg_mi = (mean(c(C3_MI, C4_MI))),
 POavg_mi = (mean(c(P3_MI, P4_MI, O1_MI, O2_MI))),
 Favg_mvl = (mean(c(F3_MVL, F4_MVL))),
 Cavg_mvl = (mean(c(C3_MVL, C4_MVL))),
 POavg_mvl = (mean(c(P3_MVL, P4_MVL, O1_MVL, O2_MVL))))
#> view(Zhang2020_avg)
```
...
#| message: false
#| echo: false
variables <- c("Favg_phase", "Cavg_phase", "POavg_phase")
es <- data.frame()
for (var in variables) {
ev <- circular_cor(Zhang2020_avg[[var]], Zhang2020_avg$PBO.OVERNIGHT.RETENTION)
es <- rbind(es, ev)
}
rownames(es) <- c("Frontal", "Central", "Parietal and Occipital")
knitr::kable(es, format = "markdown", caption = "Zhang 2020 CP Phase and Memory Circular Linear Correlation Table")
```

```{r}
Zhang2020_avg_mi <- remove_outliers(Zhang2020_avg, scale_columns =  c("Favg_mi","Cavg_mi","POavg_mi"))
Zhang2020_avg_mvl <- remove_outliers(Zhang2020_avg, scale_columns =  c("Favg_mvl","Cavg_mvl","POavg_mvl"))
```
...
# Calculate correlation coefficients between coupling modulation index and memory in each channel location
cor_mi <- c(
  "Frontal" = cor(Zhang2020_avg_mi$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mi$Favg_mi, use = "complete"),
  "Central" = cor(Zhang2020_avg_mi$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mi$Cavg_mi, use = "complete"),
  "Parietal and Occipital" = cor(Zhang2020_avg_mi$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mi$POavg_mi, use = "complete"))

# Create the table
cortable("Zhang 2020 (MI)", 3, flip = FALSE, "Correlation" = cor_mi)

# Calculate correlation coefficients between coupling mean vector length and memory in each channel location
cor_mvl <- c(
  "Frontal" = cor(Zhang2020_avg_mvl$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mvl$Favg_mvl, use = "complete"),
  "Central" = cor(Zhang2020_avg_mvl$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mvl$Cavg_mvl, use = "complete"),
  "Parietal and Occipital" = cor(Zhang2020_avg_mvl$PBO.OVERNIGHT.RETENTION ~ Zhang2020_avg_mvl$POavg_mvl, use = "complete"))

favstats(Zhang2020_avg_mi$Favg_mi)
favstats(Zhang2020_avg_mi$Cavg_mi)
favstats(Zhang2020_avg_mi$POavg_mi)
# Create the table
cortable("Zhang 2020 (MVL)", 3, flip = FALSE, "Correlation" = cor_mvl)
```

```{r}
Zhangpct_avg <- Zhangpct |>
  rowwise() |>
  mutate(
    Favg_pct = {
      ratios <- c(F3_sosp/F3_sp, Fz_sosp/Fz_sp, F4_sosp/F4_sp)
      ratios[is.infinite(ratios)] <- 10
      mean(ratios, na.rm = TRUE)
    },
    Cavg_pct = {
      ratios <- c(C3_sosp/C3_sp, Cz_sosp/Cz_sp, C4_sosp/C4_sp)
      ratios[is.infinite(ratios)] <- 10
      mean(ratios, na.rm = TRUE)
    },
    POavg_pct = {
      ratios <- c(P3_sosp/P3_sp, Pz_sosp/Pz_sp, P4_sosp/P4_sp, O1_sosp/O1_sp, O2_sosp/O2_sp)
      ratios[is.infinite(ratios)] <- 10
      mean(ratios, na.rm = TRUE)
    }
  )
#> view(Zhangpct_avg)
is_odd <- seq_len(nrow(Zhangpct_avg)) %% 2 == 1
is_even <- seq_len(nrow(Zhangpct_avg)) %% 2 == 0

zhangpct_avg_nrem2_raw <- Zhangpct_avg[is_odd, ]
zhangpct_avg_sws_raw <- Zhangpct_avg[is_even, ]
```
...
#| message: false
#| echo: false
zhangpct_avg_nrem2 <- remove_outliers(zhangpct_avg_nrem2_raw, scale_columns =  c("Favg_pct","Cavg_pct","POavg_pct"))

rows2del_nrem2 <- c(10, 22, 25)
Zhang2020_avg_nrem2 <- Zhang2020_avg[-rows2del_nrem2, ]

# Calculate correlation coefficients between coupling percentage in nREM2 stage and memory in each channel location
cor_nrem2 <- c(
  "Frontal" = cor(Zhang2020_avg_nrem2$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_nrem2$Favg_pct, use = "complete"),
  "Central" = cor(Zhang2020_avg_nrem2$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_nrem2$Cavg_pct, use = "complete"),
  "Parietal and Occipital" = cor(Zhang2020_avg_nrem2$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_nrem2$POavg_pct, use = "complete"))

zhangpct_avg_sws <- remove_outliers(zhangpct_avg_sws_raw, scale_columns =  c("Favg_pct","Cavg_pct","POavg_pct"))

rows2del_sws <- c(16, 22, 23)
Zhang2020_avg_sws <- Zhang2020_avg[-rows2del_sws, ]

# Calculate correlation coefficients between coupling percentage in nREM2 stage and memory in each channel location
cor_sws <- c(
  "Frontal" = cor(Zhang2020_avg_sws$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_sws$Favg_pct, use = "complete"),
  "Central" = cor(Zhang2020_avg_sws$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_sws$Cavg_pct, use = "complete"),
  "Parietal and Occipital" = cor(Zhang2020_avg_sws$PBO.OVERNIGHT.RETENTION ~ zhangpct_avg_sws$POavg_pct, use = "complete"))

# Create the table
cortable("Zhang 2020", 4, flip = FALSE, "nREM2" = cor_nrem2, "sws" = cor_sws)
```
...
#| include: false
## Import source data from Zhang 2020
Nicolas2022_sleep_raw <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Nicolas2022/Spindles2share.csv")
Nicolas2022_mem_raw <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Nicolas2022/offlingains2share.csv")
Nicolas2022_phase <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Nicolas2022/Nicolas_phase.csv")
```

```{r}
Nicolas2022_sleep <- data.frame()

for (row in Nicolas2022_sleep_raw) {
  row_data <- str_split_fixed(row, ",", 12)
  Nicolas2022_sleep <- rbind(Nicolas2022_sleep, as.data.frame(row_data))
}
colnames <- c('Sub','Channel','Count','Density','Duration','Amplitude','RMS','AbsPower','RelPower','Frequency','Oscillations','Symmetry')
colnames(Nicolas2022_sleep) <- colnames

Nicolas2022_sleep <- Nicolas2022_sleep[-1, ]
#> view(Nicolas2022_sleep)

Nicolas2022_sleep_wide <- Nicolas2022_sleep |>
  pivot_wider(names_from = Channel,
              values_from = Amplitude,
              id_cols = Sub)
Nicolas2022_sleep_wide$C3 <- c(60.87314849, Nicolas2022_sleep_wide$C3[-1])
Nicolas2022_sleep_wide <- Nicolas2022_sleep_wide |>
  mutate_all(~ as.numeric(.))
#> view(Nicolas2022_sleep_wide)
```

```{r}
Nicolas2022_mem <- data.frame()

for (row in Nicolas2022_mem_raw) {
  row_data <- str_split_fixed(row, ",", 5)
  Nicolas2022_mem <- rbind(Nicolas2022_mem, as.data.frame(row_data))
}
colnames <- c('Sub', 'Sequence', 'Condition', 'Time', 'Gain_RT')
colnames(Nicolas2022_mem) <- colnames

Nicolas2022_mem_wide <- Nicolas2022_mem |>
  pivot_wider(names_from = c("Condition", "Time"),
              values_from = Gain_RT,
              id_cols = Sub) |>
  rename(react_early = ` react_early`,
         react_late = ` react_late`,
         notreact_early = ` notReact_early`,
         notreact_late = ` notReact_late`)
#> view(Nicolas2022_mem_wide)

Nicolas2022_mem_wide$notreact_late <- as.numeric(Nicolas2022_mem_wide$notreact_late)
Nicolas2022_mem_wide$notreact_early <- as.numeric(Nicolas2022_mem_wide$notreact_early)
Nicolas2022_mem_wide <- Nicolas2022_mem_wide |>
  mutate(notreact_retention = (notreact_late - notreact_early))
```
...
mutate(
 Favg = Fz,
 Cavg = mean(c(C3, C4, Cz), na.rm = TRUE),
 POavg = mean(c(Pz, Oz), na.rm = TRUE))

amp <- remove_outliers(Nicolas2022_sleep_wide, scale_columns = c("Favg", "Cavg", "POavg"), memory = Nicolas2022_mem_wide)
Nicolas2022_sleep_wide <- amp$sleepchar_rem
Nicolas2022_mem_wide <- amp$memory_rem
```
...
#| echo: false
## Calculate the circular linear correlation
view(Nicolas2022_phase)
view(Nicolas2022_mem_wide)
nicolas_phase <- circular_cor(Nicolas2022_phase$phase, Nicolas2022_mem_wide$notreact_retention)
rownames(nicolas_phase) <- 'Correlation'
knitr::kable(nicolas_phase,format ="markdown",caption ="Nicolas 2022 CP Phase and Memory Circlin R Correlation Table")
```

```{r}
# Calculate correlation coefficients between spindle amplitude and memory in each channel location
cor <- c(
  "Frontal" = cor(Nicolas2022_sleep_wide$Favg ~ Nicolas2022_mem_wide$notreact_retention, use = "complete"),
  "Central" = cor(Nicolas2022_sleep_wide$Cavg ~ Nicolas2022_mem_wide$notreact_retention, use = "complete"),
  "Parietal and Occipital" = cor(Nicolas2022_sleep_wide$POavg ~ Nicolas2022_mem_wide$notreact_retention, use = "complete"))

# Create the table
cortable("Nicolas 2022", 2, flip = FALSE, "Correlation" = cor)
```
...
#| include: false
## Import source data from Zhang 2020
Niknazar2015_phase <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Niknazar2015/Sourcedata/niknazar_phase.csv")
```
...
#| echo: false
## Calculate the circular linear correlation
niknazar_phase <- circular_cor(Niknazar2015_phase$phase, Niknazar2015_phase$improvement)
rownames(niknazar_phase) <- 'Correlation'
knitr::kable(niknazar_phase,format ="markdown",caption ="Niknazar 2015 CP Phase and Memory Circlin R Correlation Table")
```
...
#| message: false
#| include: false
## Import source data from Perrault 2019
Perrault2019 <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Perrault2019/Sourcedata/Perrault_data.csv")
```

```{r}
Perrault2019 <- Perrault2019[-c(14, 17), ]
Perrault2019 <- Perrault2019 |>
  mutate(N2_Fz_phase = deg2rad(CFC_N2_Fz_prefangle_dg - 270),
         N2_Pz_phase = deg2rad(CFC_N2_Pz_prefangle_dg - 270),
         N3_Fz_phase = deg2rad(CFC_N3_Fz_prefangle_dg - 270),
         N3_Pz_phase = deg2rad(CFC_N3_Pz_prefangle_dg - 270))
```

```{r}
variables <- c("N2_Fz_phase", "N2_Pz_phase", "N3_Fz_phase", "N3_Pz_phase")
es_phase <- data.frame()
for (var in variables) {
ev_phase <- circular_cor(Perrault2019[[var]], Perrault2019$MEMO_overnight_MOminusEV_accuracy_CminusW)
es_phase <- rbind(es_phase, ev_phase)
}
rownames(es_phase) <- c("N2 Frontal", "N2 Parietal", "N3 Frontal", "N3 Parietal")
knitr::kable(es_phase, format = "markdown", caption = "Perrault 2019 CP Phase and Memory Circular Linear Correlation Table")

mean(circular(Perrault2019$N2_Fz_phase, units = "radians", template = "geographics"))
favstats(Perrault2019$N2_Fz_phase)
mean(circular(Perrault2019$N2_Pz_phase, units = "radians", template = "geographics"))
mean(circular(Perrault2019$N3_Fz_phase, units = "radians", template = "geographics"))
mean(circular(Perrault2019$N3_Pz_phase, units = "radians", template = "geographics"))
```
...
#| message: false
#| echo: false
Perrault2019_amp <- remove_outliers(Perrault2019, scale_columns = c("Spindle_Fz_N2_PeakAmp_uV", "Spindle_Pz_N2_PeakAmp_uV", "Spindle_Fz_N3_PeakAmp_uV", "Spindle_Pz_N3_PeakAmp_uV"))

# Calculate correlation coefficients between spindles and memory in each channel location
cor_amp <- c(
  "N2 Frontal" = cor(Perrault2019_amp$Spindle_Fz_N2_PeakAmp_uV ~ Perrault2019_amp$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N2 Parietal" = cor(Perrault2019_amp$Spindle_Pz_N2_PeakAmp_uV ~ Perrault2019_amp$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N3 Frontal" = cor(Perrault2019_amp$Spindle_Fz_N3_PeakAmp_uV ~ Perrault2019_amp$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N3 Parietal" = cor(Perrault2019_amp$Spindle_Pz_N3_PeakAmp_uV ~ Perrault2019_amp$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"))

# Create the table
cortable("Perrault 2019", 2, flip = FALSE, "Correlation" = cor_amp)
```
...
#| message: false
#| echo: false
Perrault2019_str <- remove_outliers(Perrault2019, scale_columns = c("CFC_N2_Fz_SI_mag", "CFC_N2_Pz_SI_mag", "CFC_N3_Fz_SI_mag", "CFC_N3_Pz_SI_mag"))

# Calculate correlation coefficients between spindles and memory in each channel location
cor_str <- c(
  "N2 Frontal" = cor(Perrault2019_str$CFC_N2_Fz_SI_mag ~ Perrault2019_str$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N2 Parietal" = cor(Perrault2019_str$CFC_N2_Pz_SI_mag ~ Perrault2019_str$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N3 Frontal" = cor(Perrault2019_str$CFC_N3_Fz_SI_mag ~ Perrault2019_str$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"),
  "N3 Parietal" = cor(Perrault2019_str$CFC_N3_Pz_SI_mag ~ Perrault2019_str$MEMO_overnight_MOminusEV_accuracy_CminusW, use = "complete"))

# Create the table
cortable("Perrault 2019", 3, flip = FALSE, "Correlation" = cor_str)
```
