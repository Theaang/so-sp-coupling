...
Nicolas2022_cp_raw <- read.csv("https://raw.githubusercontent.com/Theaang/so-sp-coupling/main/Paper/Nicolas2022/mvl_pp_epoch30_2share.csv")
```
...
```{r}
Nicolas2022_cp <- data.frame()

for (row in Nicolas2022_cp_raw) {
  row_data <- str_split_fixed(row, ";", 4)
  Nicolas2022_cp <- rbind(Nicolas2022_cp, as.data.frame(row_data))
}
colnames <- c('mean_vector_length', 'preferred_phase', 'subject', 'channel')
colnames(Nicolas2022_cp) <- colnames

Nicolas2022_cp_wide <- Nicolas2022_cp |>
  pivot_wider(names_from = channel,
              values_from = c('mean_vector_length', 'preferred_phase'),
              id_cols = subject)
#> view(Nicolas2022_cp_wide)

Nicolas2022_cp_wide <- Nicolas2022_cp_wide |>
